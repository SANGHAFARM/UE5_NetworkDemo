...
void AABCharacterPlayer::SetDead()
{
	...
	GetWorld()->GetTimerManager().SetTimer(DeadTimerHandle, this, &AABCharacterPlayer::ResetPlayer, PlayerRespawnTime, false);
...
void AABCharacterPlayer::Attack()
{
	...
	if (bCanAttack)
	{
		...
		if (!HasAuthority())
		{
			...
			// 고려해야 할 것:
			// 캐릭터 무브먼트 설정이나 공격 시간 등은 클라이언트가 악의적으로 변경(변조) 할 수 있다는 사실을 가정해야 함
...
void AABCharacterPlayer::ServerRPCAttack_Implementation(float AttackStartTime)
{
	...
	// 예외처리
	// 네트워크 상태가 너무 안 좋은 경우, AttackTimeDifference 값이 너무 크게 되면,
	...
	for (APlayerController* PlayerController : TActorRange<APlayerController>(GetWorld()))
	{
		...
		if (PlayerController && GetController() != PlayerController)
		{
			...
			{
				// 여기로 넘어온 플레이어 컨트롤러는
				// 서버도 아니고, 본인 클라이언트도 아닌 나머지 클라이언트
...
void AABCharacterPlayer::MulticastRPCAttack_Implementation()
{
	...
	// 공격 상태 값과 무브먼트 모드 값은 프로퍼니 리플리케이션으로 전달되어 처리가 될 것
	// 하지만, 공격 애니메이션은 재생하지 않았기 때문에 재생해야 함
	// 나머지 클라이언트인지 파악
	if (!IsLocallyControlled())
	{
		PlayAttackAnimation();
...
void AABCharacterPlayer::ServerRPCNotifyHit_Implementation(const FHitResult& HitResult, float HitCheckTime)
{
	...
	if(IsValid(HitActor))
	{
		...
		DrawDebugPoint(GetWorld(), ActorBoxCenter, 30.f, FColor::Magenta, false, 5.f);
#endif
